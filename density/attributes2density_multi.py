...
def iterate(val):
    ...
    clipData = driver.Open("./tmp/clip.shp",1)
    clipLayer = clipData.GetLayer()
    pointData = driver.Open("./tmp/point.shp")
    pointLayer = pointData.GetLayer()
    feature = clipLayer.GetFeature(val)
    ...
    print val
    sumattr=0
    if zfield and count>0:
        for point in pointLayer:
            attr = point.GetField(zfield)
            try:
                sumattr+=float(attr)
            except:
                print "Invalid attr"
...
def main(argv):
    ...
    #Write DBF
    os.makedirs("./tmp")
    ogr2ogr.main(["","-f","ESRI Shapefile","./tmp/point.dbf",options.pointInput])
    #Write VRT
    print "Writing CSV VRT..."
    vrt = open('./tmp/'+inputname+'.vrt','w')
    vrt.write("<OGRVRTDataSource>\n")
    vrt.write("\t<OGRVRTLayer name='point'>\n")
    ...
    return (options.zfield,options.cpus)

if __name__ == "__main__":
    (zfield,cpus) =  main(sys.argv[1:])
    print "Calculating point density..."
    driver = ogr.GetDriverByName("ESRI Shapefile")
    clipData = driver.Open("./tmp/clip.shp",1)
    clipLayer = clipData.GetLayer()
    featureCount = clipLayer.GetFeatureCount()
    print "Iterating over "+str(featureCount)+" features..."
    pool = multiprocessing.Pool(cpus)
    pool.map(iterate,range(0, featureCount))